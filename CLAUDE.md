...
- **ASR is platform-split** in `zimakuou/transcribe.py`, same pattern as the translator:
  ...
  - **Recommended speed/quality alternative**: `--asr-model mobiuslabsgmbh/faster-whisper-large-v3-turbo` swaps in OpenAI's own 4-layer-decoder distillation of large-v3. ~3× faster decoding than full large-v3 with comparable recall on mixed-source JA video (WBS-style news with interviews / B-roll / phone audio). Works through the existing CT2 path with no code changes; uses the same tightened VAD and word-timestamp settings since turbo retains enough cross-attention heads for `model.align()`.
