...
- **ASR is platform-split** in `zimakuou/transcribe.py`, same pattern as the translator:
  - **macOS (Apple Silicon)**: `mlx-whisper` — Metal GPU, fastest on this hardware. Default `mlx-community/whisper-large-v3-turbo` — MLX conversion of OpenAI's 4-decoder-layer distillation of large-v3. ~3× faster decoding than full large-v3 with comparable recall on mixed-source JA video; retains enough cross-attention heads for word-timestamp alignment, so `split_long_cue` still works. Pass `--asr-model mlx-community/whisper-large-v3-mlx` if you want the full 32-decoder-layer large-v3 for hard audio.
  - **Windows / non-Apple**: `faster-whisper` (CTranslate2). Default `mobiuslabsgmbh/faster-whisper-large-v3-turbo` — CT2 conversion of OpenAI's 4-decoder-layer distillation of large-v3. ~3× faster decoding than full large-v3 with comparable recall on mixed-source JA video (WBS-style news with interviews / B-roll / phone audio); retains enough cross-attention heads for `model.align()`, so the existing tightened VAD and word-timestamp settings still work. CT2 has no Metal backend, so on Mac it'd be CPU-only — that's why we don't use it here. Pass `--asr-model Systran/faster-whisper-large-v3` if you want the full 32-decoder-layer large-v3 for hard audio.
  ...
  - We also tried NVIDIA NeMo Parakeet (`nvidia/parakeet-tdt_ctc-0.6b-ja`) and removed it. Three problems on real-world JA video: (1) the transducer decoder silently emits blank tokens on anything outside its training distribution (ReazonSpeech / broadcast news), so on mixed-source video — interview audio, B-roll with BGM, telephone-quality remotes — large stretches of dialogue went un-transcribed; (2) no text-prior input means `initial_prompt` biasing is dropped, hurting proper-noun recall; (3) timestamps are unreliable for cue splitting — `timestamp.word` collapses to a single chunk-spanning entry and `timestamp.char` has populated timings but empty `char` text fields, so we couldn't map silence gaps to text positions. Net result: worse recall and worse cue boundaries than `faster-whisper-large-v3` on the same source. The reviews praising Parakeet were benchmark WER on clean speech, not real-world recall — different metric.
  - We also tried `kotoba-tech/kotoba-whisper-v2.0-faster` and removed any model-specific handling. It's built on `distil-large-v3` (2 decoder layers vs. 32 in large-v3) and fine-tuned on the same ReazonSpeech corpus as Parakeet, so it inherits the same "skip uncertain audio" failure mode plus a `model.align()` crash on `word_timestamps=True` (distil drops the cross-attention heads faster-whisper needs). Relaxing VAD helped marginally but didn't close the recall gap — the missing dialogue is the architecture and training-data choice, not the pre-filter. v2.1 (stable-ts) and v2.2 (punctuation + diarization) share the same backbone, so they don't fix recall. **Use `mobiuslabsgmbh/faster-whisper-large-v3-turbo` instead** if you want speed — different distillation (OpenAI's own, 4 decoder layers) that retains recall, no special-casing needed.
- **Audio extraction**: shell out to `ffmpeg` (must be on PATH). Target 16kHz mono WAV.
- **Translation LLM** is the [SakuraLLM Sakura-7B-Qwen2.5-v1.0](https://huggingface.co/SakuraLLM/Sakura-7B-Qwen2.5-v1.0-GGUF) GGUF (IQ4_XS, ~4.3 GB), served via `llama-cpp-python` with Metal on macOS and CUDA/CPU on Windows. Sakura is fine-tuned on light-novel / galge / anime JP→ZH and uses a specific Chinese prompt format with a `gpt_dict` glossary block in the user message — that wiring lives in `zimakuou/translators/_prompt.py` (`SAKURA_SYSTEM`, `build_sakura_prompt`). The 7B and 14B v1.0 releases share the same system prompt and `gpt_dict` format, so switching between them is a one-flag change (`--llm SakuraLLM/Sakura-14B-Qwen2.5-v1.0-GGUF`). Sakura is trained to emit Simplified Chinese; the OpenCC `s2twp` post-pass in `translate.py` normalises to Traditional, and `Context.apply_glossary` enforces canonical glossary terms after that.
  - **7B vs 14B tradeoff** (benchmarked on a 50-cue WBS slice, RTX 4060 8 GB): 7B IQ4_XS fits entirely on GPU (`n_gpu_layers=-1`) and runs at ~3 cues/sec; 14B Q4_K_M is 8.4 GB so it partial-offloads (`n_gpu_layers=35`) and runs at ~0.65 cues/sec — **~4.7× speedup for the 7B**. Quality cost: ~1 meaning-distorting mistranslation per ~12 cues on the 7B (long subordinate clauses dropped, occasional idiom misread) plus more proper-noun drift — the glossary post-pass mitigates the latter but not the former. Default is 7B; use `--llm SakuraLLM/Sakura-14B-Qwen2.5-v1.0-GGUF --n-gpu-layers 35` when you want canon-quality output.
